...
const DataTableFilter = <TData,>({
  ...
  const hasValue = React.useMemo(() => {
    ...
    if (typeof filter === "object" && filter !== null) {
      ...
      return (
        keys.length > 0 &&
        (filter as any)[keys[0]] !== null &&
        (filter as any)[keys[0]] !== undefined
      );
    }
    return true;
  }, [filter]);
  ...
  const { displayValue, isCustomRange } = React.useMemo(() => {
    ...
    if (typeof filter === "string") {
      ...
      if (!options || options.length === 0) {
        ...
        displayValue =
          options?.find((o: any) => o.value === filter)?.label ?? null;
      }
    }
    ...
    if (
      ...
      !isDateComparisonOperator(filter)
    ) {
  ...
  return (
    <Popover open={open} onOpenChange={onOpenChange} modal>
      ...
      >
        ...
        <div
          className={clx("flex items-center px-2 py-1 text-ui-fg-muted", {
            "border-r": hasValue,
          })}
        >
          {label || id}
      ...
      >
        {(() => {
          switch (type) {
            ...
            case "multiselect":
              const multiselectRest = rest as Omit<
                DataTableMultiselectFilterProps,
                "id" | "type" | "label" | "options"
              >;
              return (
...
type DataTableFilterDateContentProps<TData> = {
  id: string;
  instance: UseDataTableReturn<TData>;
  filter: unknown;
...
const DataTableFilterDateContent = <TData,>({
  id,
  filter,
  options,
  format = "date",
  ...
  onUpdate,
  instance,
}: DataTableFilterDateContentProps<TData>) => {
...
type DataTableFilterSelectContentProps<TData> = {
  id: string;
  instance: UseDataTableReturn<TData>;
  filter?: string[];
...
const DataTableFilterSelectContent = <TData,>({
  id,
  instance,
  filter = [],
  options,
  isNew = false,
  onUpdate,
}: DataTableFilterSelectContentProps<TData>) => {
...
type DataTableFilterRadioContentProps<TData> = {
  id: string;
  filter: unknown;
  instance: UseDataTableReturn<TData>;
  options: DataTableFilterOption<string>[];
...
const DataTableFilterRadioContent = <TData,>({
  id,
  filter,
  options,
  onUpdate,
  instance,
}: DataTableFilterRadioContentProps<TData>) => {
  const onValueChange = React.useCallback(
    (value: string) => {
      if (onUpdate) {
        onUpdate(value);
...
function isDateFilterProps(
  props?: unknown | null
): props is DataTableDateFilterProps {
  if (!props) {
    return false;
  }
...
function isMultiselectFilterProps(
  props?: unknown | null
): props is DataTableMultiselectFilterProps {
  if (!props) {
    return false;
  }
...
function isStringFilterProps(
  props?: unknown | null
): props is DataTableStringFilterProps {
  if (!props) {
    return false;
  }
...
function isNumberFilterProps(
  props?: unknown | null
): props is DataTableNumberFilterProps {
  if (!props) {
    return false;
  }
...
function isCustomFilterProps(
  props?: unknown | null
): props is DataTableCustomFilterProps {
  if (!props) {
    return false;
  }
...
type DataTableFilterMultiselectContentProps<TData> = {
  id: string;
  instance: UseDataTableReturn<TData>;
  filter?: string[];
  options: DataTableFilterOption<string>[];
...
const DataTableFilterMultiselectContent = <TData,>({
  id,
  filter = [],
  options,
  searchable = true,
  onUpdate,
  instance,
}: DataTableFilterMultiselectContentProps<TData>) => {
  const [search, setSearch] = React.useState("");
...
type DataTableFilterStringContentProps<TData> = {
  id: string;
  instance: UseDataTableReturn<TData>;
  filter?: string;
  placeholder?: string;
...
const DataTableFilterStringContent = <TData,>({
  id,
  filter,
  placeholder = "Enter value...",
  onUpdate,
  instance,
}: DataTableFilterStringContentProps<TData>) => {
...
type DataTableFilterNumberContentProps<TData> = {
  id: string;
  instance: UseDataTableReturn<TData>;
  filter: any;
  placeholder?: string;
...
const DataTableFilterNumberContent = <TData,>({
  id,
  filter,
  ...
  onUpdate,
  instance,
}: DataTableFilterNumberContentProps<TData>) => {
  ...
  const handleOperatorChange = React.useCallback(
    (newOperator: string) => {
      ...
      if (!isNaN(num)) {
        const filterValue =
          includeOperators && newOperator !== "eq"
            ? { [`$${newOperator}`]: num }
            : num;
...
type DataTableFilterCustomContentProps<TData> = {
  id: string;
  instance: UseDataTableReturn<TData>;
  filter: any;
  onRemove: () => void;
...
const DataTableFilterCustomContent = <TData,>({
  id,
  filter,
  onRemove,
  render,
  onUpdate,
  instance
}: DataTableFilterCustomContentProps<TData>) => {
